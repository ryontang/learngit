...
int OperatorIni::ReadINI(string path)
{
    ...
    while (getline(in_conf_file, str_line))
    {
        ...
        string str_key = ""; string str_value = "";
...
string OperatorIni::GetValue(string root, string key)
{
...
int OperatorIni::WriteINI(string path)
{
    ...
    } out_conf_file.close(); out_conf_file.clear();
    return 1;
}

vector<ININode>::size_type OperatorIni::SetValue(string root, string key, string value)
{
    map<string, SubNode>::iterator itr = map_ini.find(root);
    //查找
    if (map_ini.end() != itr)
